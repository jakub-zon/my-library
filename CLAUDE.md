...
- **Read date** is NOT in the main listing card. It's rendered only on the per-shelf URL `https://lubimyczytac.pl/profil/<userId>/<nick>/biblioteczka/lista?shelfs=<shelfId>` inside `div.book-card__read-dates` (text "Przeczytał: <br/>YYYY-MM-DD"). Scraper does a second pass on the Przeczytane shelf (id 6037367 here) and merges by book id. Books without a manually set date on LC simply don't get the field.
- **Listing card selectors (post-2026-05 redesign):** card root `div.book-card` with `data-book-id="<id>"`; title `a.book-card__title`; authors `.book-card__author a`; LC avg rating `.book-card__detail--rating .rating__avarage`; user rating (Przeczytane only) `.book-card__detail--user-rating .rating__avarage`; shelves `.book-card__shelves a.book-card__shelf`; cycle `.book-card__detail--cycle a`; cover `img.book-card__cover-image`. Old `authorAllBooks__*` / `listLibrary__*` classes are gone — if extractions return 0 books on a 200 OK page with a working paginator, LC has reskinned again. Paginator total still on `input.jsPagerInput[data-maxpage]`.
